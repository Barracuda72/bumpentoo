...
inherit eutils flag-o-matic multilib toolchain-funcs eapi7-ver
...
SRC_URI="http://www.festvox.org/packed/festival/${MY_PV}/${MY_P}-release.tar.gz"
...
src_configure() {
	...
	sed -i -e 's/@COMPILERTYPE@/gcc/' ${CONFIG} || die
	sed -i -e 's/^#SHARED/SHARED/' ${CONFIG} || die
	sed -i -e 's/^TERMCAPLIB =/\0 -ltinfo/' ${CONFIG} || die
	if use nas; then
...
src_compile() {
	emake -j1 CC="$(tc-getCC -fPIC)" CXX="$(tc-getCXX -fPIC)" CXX_OTHER_FLAGS="${CXXFLAGS}" CC_OTHER_FLAGS="${CFLAGS}" \
		LDFLAGS="${LDFLAGS}"
}
